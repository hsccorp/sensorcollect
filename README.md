...
Cloud Database
---------------
* At the end of the trip, the app will automatically try to uplaod the trip to a preconfigured Firebase DB, for which you need credentails, which you don't have unless you are an HSC employee. That is intentional. You can either modify the code to use your own DB (see below), or, just hit cancel when you are promted for a password, in which case the trip never gets uploaded. You can still download/share it locally using the "Share" button
